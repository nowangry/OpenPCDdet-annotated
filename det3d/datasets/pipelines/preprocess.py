...
class Preprocess(object):
    def __init__(self, cfg=None, **kwargs):
        ...
        # self.no_augmentation = cfg.get('no_augmentation', False) # False
        self.no_augmentation = cfg.get('no_augmentation', True)  # adv no_augmentation
...
class Voxelization(object):
    ...
    def __call__(self, res, info):
        ...
        # 得到voxel化后的矩阵数据。（[M, max_points, ndim]）
        # voxel位置映射到voxel index，即voxel-level的相对坐标 --> voxel id。（[M, 3]）
        # 每个voxel内的点云数量
...
class AssignLabel(object):
    ...
    def __call__(self, res, info):
        ...
        # cfg = global_var.get_value('global_cfg')

        if res["mode"] == "train":
            # if res["mode"] == "train" or cfg.adv_flag:
            ...
            gt_dict = res["lidar"]["annotations"]
            res["lidar"]["gt_dict_origin"] = gt_dict
            ...
            for idx, task in enumerate(self.tasks):
                ...
                for k in range(num_objs):
                    ...
                    if w > 0 and l > 0:
                        ...
                        # 原点为voxel坐标系原点，大小比例转换为heatmap上的大小，单位为voxel
                        ...
                        if res['type'] == 'NuScenesDataset': 
                            ...
                            anno_box[new_idx] = np.concatenate(
                                (ct - (x, y), z, np.log(gt_dict['gt_boxes'][idx][k][3:6]),  # ct - (x, y) : location bia
                                 np.array(vx), np.array(vy), np.sin(rot), np.cos(rot)), axis=None)  # x, y
                        elif res['type'] == 'WaymoDataset':
                            vx, vy = gt_dict['gt_boxes'][idx][k][6:8]
        ...
        res["lidar"]["targets"] = example  # 生成"targets"
