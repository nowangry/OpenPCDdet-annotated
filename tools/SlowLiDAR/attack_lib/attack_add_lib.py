...
class PointAddAttacker:
    ...
    def get_critical_points(self, model, lidar_points, n_step=30, n_probe=3000):
        ...
        for i in range(n_step):
            ...
            point_data_cube = pixor_preprocess(all_points)  # Change original LiDAR points to BEV maps

            # forward passing
            pred = model(point_data_cube.unsqueeze(0))

            before_corners, after_corners = filter_pred(pred, self.cls_threshold, self.nms_iou_threshold,
                                                        self.nms_top)  # NMS
