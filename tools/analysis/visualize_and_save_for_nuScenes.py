...
NUSCENSE_CLASSES = ['car', 'truck', 'construction_vehicle', 'bus', 'trailer', 'barrier', 'motorcycle', 'bicycle',
                    'pedestrian', 'traffic_cone', 'ignore']
rgb_colors = ncolors(len(NUSCENSE_CLASSES))
target_key = None
json_path = None
window_width = 1920
window_height = 1080
...
def generate_images_with_pred_3dbox(dir_save_images, pc_dir, pkl_path, thresh=0.3):
    ...
    for key, data in data_dicts.items():
        ...
        ## innocent
        if 'innocent' in dir_save_images:
...
def generate_images_without_3dbox(dir_save_images, pc_dir, pkl_path, thresh=0.3):
    ...
    for key, data in data_dicts.items():
        ...
        #     front = json_data['trajectory'][0]['front']
        #     lookat = json_data['trajectory'][0]['lookat']
        #     up = json_data['trajectory'][0]['up']
        #     zoom = json_data['trajectory'][0]['zoom']
...
def generate_images_with_modification_positions(dir_save_images, pc_dir, pkl_path, thresh=0.3, is_heatmap=False):
    ...
    for key, data in data_dicts.items():
        ...
        if is_heatmap:
            ...
            # colors = color_generation_black2white(np.abs(points_diff[:, :3]).sum(-1))
        else:
            cnt_modify = 0
...
def generate_images_with_gt_3dbox(dir_save_images, pc_dir, pkl_path, thresh=0.3):
    ...
    ## ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    gt_pkl_path = os.path.join('/home/jqwu/Datasets/nuScenes/v1.0-mini/',
        ...
        for scence in gt_data_dicts_origin:
            if scence['token'] in data_dicts:
                ...
                ob['label_preds'] = torch.from_numpy(
                    np.array([NUSCENSE_CLASSES.index(name) for i, name in enumerate(box_name)]).reshape(-1))
...
def visualize_points_saved_nuScenes(dir_save_images, pc_dir, pkl_path, thresh=0.3):
...
if __name__ == '__main__':
    ...
    ## innocent
    innocent_voxel = dict(
        dir_save_images=r'./save/images-nuScenes-voxel/innocent',
        pc_dir=r'/Data4T/Outputs/centerpoint-adv/AdaptiveEPS/strategy_PGD-filterOnce-fixedEPS_0.3-eps_0.5-eps_ratio_0.5-num_steps_10-attach_rate_0.2/',
        work_dir=r'/home/jqwu/Codes/CenterPoint-adv/work_dirs/v1.0-mini/Eval/innocent/nusc_centerpoint_pp_02voxel_two_pfn_10sweep-innocent/',
    )

    ## PGD
    PGD_voxel = dict(
        dir_save_images=r'./save/images-nuScenes-voxel/PGD',
        pc_dir=r'/Data4T/Outputs/centerpoint-adv/PGD_CoorAdjust/eps_0.2-eps_iter_0.03-num_steps_10-randStart/',
        work_dir=r'/home/jqwu/Codes/CenterPoint-adv/work_dirs/v1.0-mini/ADV_reVoxelize/PGD_CoorAdjust/eps_0.2-eps_iter_0.03-num_steps_10-randStart/',
    )

    ## IOU
    IOU_voxel = dict(
        dir_save_images=r'./save/images-nuScenes-voxel/IOU',
        pc_dir=r'/Data4T/Outputs/centerpoint-adv/IOU_iter/eps_0.2-eps_iter_0.2-num_steps_1-Lambda_0.1-iou_0.1-score_0.1/',
        work_dir=r'/home/jqwu/Codes/CenterPoint-adv/work_dirs/v1.0-mini/ADV_reVoxelize/IOU_iter/eps_0.2-eps_iter_0.2-num_steps_1-Lambda_0.1-iou_0.1-score_0.1',
    )

    ## AdaptiveEPS
    AdaptiveEPS_voxel = dict(
        dir_save_images=r'./save/images-nuScenes-voxel/AdaptiveEPS',
        pc_dir=r'/Data4T/Outputs/centerpoint-adv/AdaptiveEPS/strategy_PGD-filterOnce-fixedEPS_0.3-eps_0.5-eps_ratio_0.5-num_steps_10-attach_rate_0.2/',
        work_dir=r'/home/jqwu/Codes/CenterPoint-adv/work_dirs/v1.0-mini/ADV_reVoxelize/AdaptiveEPS/strategy_PGD-filterOnce-fixedEPS_0.3-eps_0.5-eps_ratio_0.5-num_steps_10-attach_rate_0.2',
    )
    ...
    ## PGD
    PGD_pillar = dict(
        dir_save_images=r'./save/images-nuScenes-pillar/PGD',
        pc_dir=r'/Data4T/Outputs/centerpoint-adv-pillar/PGD_CoorAdjust/eps_0.2-eps_iter_0.03-num_steps_10-randStart/',
        work_dir=r'/home/jqwu/Codes/CenterPoint-adv/work_dirs/v1.0-mini/ADV_reVoxelize-pillar/PGD_CoorAdjust/eps_0.2-eps_iter_0.03-num_steps_10-randStart/',
    )

    ## IOU
    IOU_pillar = dict(
        dir_save_images=r'./save/images-nuScenes-pillar/IOU',
        pc_dir=r'/Data4T/Outputs/centerpoint-adv-pillar/IOU_iter/eps_0.2-eps_iter_0.2-num_steps_1-Lambda_0.1-iou_0.1-score_0.1/',
        work_dir=r'/home/jqwu/Codes/CenterPoint-adv/work_dirs/v1.0-mini/ADV_reVoxelize-pillar/IOU_iter/eps_0.2-eps_iter_0.2-num_steps_1-Lambda_0.1-iou_0.1-score_0.1',
    )

    ## AdaptiveEPS
    AdaptiveEPS_pillar = dict(
        dir_save_images=r'./save/images-nuScenes-pillar/AdaptiveEPS',
        pc_dir=r'/Data4T/Outputs/centerpoint-adv-pillar/AdaptiveEPS/strategy_PGD-filterOnce-fixedEPS_0.3-eps_0.5-eps_ratio_0.5-num_steps_10-attach_rate_0.3/',
        work_dir=r'/home/jqwu/Codes/CenterPoint-adv/work_dirs/v1.0-mini/ADV_reVoxelize-pillar/AdaptiveEPS/strategy_PGD-filterOnce-fixedEPS_0.3-eps_0.5-eps_ratio_0.5-num_steps_10-attach_rate_0.3',
    )

    adv_list = [PGD_voxel, IOU_voxel, AdaptiveEPS_voxel]
    ...
    target_key = 'c36e8482feb041de9f081e76396cf4fe'
    json_path = '/home/jqwu/Codes/CenterPoint-adv/tools/analysis/angle_json/FangDa.json'
    ...
    for adv_alg in adv_list:
        ...
        visualize_points_saved_nuScenes(dir_save_images, pc_dir, pkl_path, thresh=0.3)
