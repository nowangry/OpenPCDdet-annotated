...
def pixor_preprocess(lidar_raw_data):
    ...
    for i in range(0, lidar_raw_data.shape[0]):
        ...
        with torch.no_grad():
            ...
            if lidar_raw_data[i][0] < x_MIN or lidar_raw_data[i][0] > x_MAX or lidar_raw_data[i][1] < y_MIN or \
                    lidar_raw_data[i][1] > y_MAX or lidar_raw_data[i][2] < z_MIN or lidar_raw_data[i][2] > z_MAX:
                continue
