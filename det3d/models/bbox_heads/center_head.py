...
class CenterHead(nn.Module):
    ...
    def forward(self, x, *kwargs):
        ret_dicts = []
        '''
        input shape: (1, 512, 180, 180)
        output shape: (1, 64, 180, 180)
        '''
    ...
    def loss(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if 'catAdv_dict' in kwargs['cfg']:
                hm_loss = 0
                # 交换类别
                catAdv_dict = kwargs['cfg'].catAdv_dict
                if catAdv_dict.swap_gt_cat:
                    if example['hm'][task_id].shape[1] > 1:
                        cat_copy = example['cat'][task_id].clone()
                        for index, is_mask in enumerate(example['mask'][task_id][0]):
                            if is_mask:
                                cat_copy[0, index] = (cat_copy[0, index] + 1) % 2
                            else:
                                break
                        hm_loss = - self.crit(preds_dict['hm'], example['hm'][task_id], example['ind'][task_id],
                                              example['mask'][task_id], cat_copy)
            ...
            if kwargs['cfg'].adv_flag:  # adv
                ret.update({'loss': loss, 'hm_loss': hm_loss, 'loc_loss': loc_loss, 'loc_loss_elem': box_loss,
    ...
    def predict(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            batch_hm = torch.sigmoid(preds_dict['hm'])  # hm转换成概率
            ...
            for k in rets[0][i].keys():
                if k in ["box3d_lidar", "scores"]:
                    ...
                    for j, num_class in enumerate(self.num_classes):
                        rets[j][i][k] += flag
                        flag += num_class  # task内相对label转换成全任务绝对label
                    ret[k] = torch.cat([ret[i][k] for ret in rets])
    ...
    def post_processing(self, batch_box_preds, batch_hm, test_cfg, post_center_range, task_id):
        ...
        for i in range(batch_size):
            ...
            scores, labels = torch.max(hm_preds, dim=-1)  # task类别内分类

            score_mask = scores > test_cfg.score_threshold  # est_cfg.score_threshold = 0.1
            distance_mask = (box_preds[..., :3] >= post_center_range[:3]).all(1) \
                            & (box_preds[..., :3] <= post_center_range[3:]).all(1)

            mask = distance_mask & score_mask
            # 使用mask过滤distance和score
