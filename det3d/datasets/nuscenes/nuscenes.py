...
class NuScenesDataset(PointCloudDataset):
    ...
    def __init__(
        ...
        # adv -------------------
        self.global_cfg = kwargs.get('global_cfg', None)
    ...
    def load_infos(self, info_path):
        ...
        if not self.test_mode and not self.is_adv:  # if training # adv
            self.frac = int(len(_nusc_infos_all) * 0.25)  # 80
    ...
    def evaluation(self, detections, output_dir=None, testset=False, **kwargs):
        # adv
        cfg = kwargs['cfg']
        if cfg.adv_flag and 'v1.0-mini' in self.version:
            version = "v1.0-mini-train"
        else:
    ...
    def evaluation_from_json(self, output_dir=None, testset=False, **kwargs):
        # adv
        cfg = kwargs['cfg']
        if cfg.adv_flag and 'v1.0-mini' in self.version:
            version = "v1.0-mini-train"
        ...
        if not testset:
            print(f"Start running eval_main to evaluate {res_path}")
