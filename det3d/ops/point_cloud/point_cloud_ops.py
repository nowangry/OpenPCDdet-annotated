...
def _points_to_voxel_reverse_kernel_for_iterAttack_momentum(
        ...
        max_points=35,
        max_voxels=20000,
        points_innocent_ori=np.array([]), voxels_innocent_ori=np.array([]),
        momentum=np.array([]), voxels_momentum=np.array([]),
):
    # put all computations to one loop.
    # we shouldn't create large array in main jit code, otherwise
    # reduce performance
...
@numba.jit(nopython=True)
def _points_to_voxel_reverse_kernel_for_iterAttack_momentum_GV(
        points,
        voxel_size,
        coors_range,
        num_points_per_voxel,
        ...
        points_innocent_ori=np.array([]), voxels_innocent_ori=np.array([]),
        momentum=np.array([]), voxels_momentum=np.array([]),
        GV_grad=np.array([]), voxels_GV_grad=np.array([]),
        grad_points=np.array([]), voxels_grad_points=np.array([]),
        points_adv=np.array([]), voxels_points_adv=np.array([]),
        v=np.array([]), voxels_v=np.array([]),
):
    # put all computations to one loop.
    # we shouldn't create large array in main jit code, otherwise
    # reduce performance
    ...
    for i in range(N):  # 遍历每个point
        ...
        if num < max_points:
            ...
            voxels_innocent_ori[voxelidx, num] = points_innocent_ori[i]
            voxels_momentum[voxelidx, num] = momentum[i]
            voxels_GV_grad[voxelidx, num] = GV_grad[i]
            voxels_grad_points[voxelidx, num] = grad_points[i]
            voxels_points_adv[voxelidx, num] = points_adv[i]
            voxels_v[voxelidx, num] = v[i]
    return voxel_num

@numba.jit(nopython=True)
def _points_to_voxel_reverse_kernel_for_iterAttack_momentum_V(
        points,
        voxel_size,
        coors_range,
        num_points_per_voxel,
...
def points_to_voxel_for_iterAttack_momentum(
    ...
    num_points_per_voxel = num_points_per_voxel[:voxel_num]
    voxels_innocent_ori = voxels_innocent_ori[:voxel_num]
    voxels_momentum = voxels_momentum[:voxel_num]
    return voxels, coors, num_points_per_voxel, voxels_innocent_ori, voxels_momentum


def points_to_voxel_for_iterAttack_momentum_GV(
        points, voxel_size, coors_range, max_points=35, reverse_index=True, max_voxels=20000,
        points_innocent_ori=np.array([]), momentum=np.array([]),
        GV_grad=np.array([]), grad_points=np.array([]), points_adv=np.array([]), v=np.array([]),
):
    """convert kitti points(N, >=3) to voxels. This version calculate
    everything in one loop. now it takes only 4.2ms(complete point cloud)
    with jit and 3.2ghz cpu.(don't calculate other features)
    ...
    voxels_GV_grad = np.zeros(
        shape=(max_voxels, max_points, points.shape[-1]), dtype=points.dtype
    )
    voxels_grad_points = np.zeros(
        shape=(max_voxels, max_points, points.shape[-1]), dtype=points.dtype
    )
    voxels_points_adv = np.zeros(
        shape=(max_voxels, max_points, points.shape[-1]), dtype=points.dtype
    )
    voxels_v = np.zeros(
        shape=(max_voxels, max_points, points.shape[-1]), dtype=points.dtype
    ...
    coors = np.zeros(shape=(max_voxels, 3), dtype=np.int32)
    if reverse_index:
        voxel_num = _points_to_voxel_reverse_kernel_for_iterAttack_momentum_GV(
            points,
            voxel_size,
            coors_range,
            num_points_per_voxel,
            ...
            max_points,
            max_voxels,
            points_innocent_ori, voxels_innocent_ori,
            momentum, voxels_momentum,
            GV_grad, voxels_GV_grad,
            grad_points, voxels_grad_points,
            points_adv, voxels_points_adv,
            v, voxels_v
        )
    ...
    voxels_innocent_ori = voxels_innocent_ori[:voxel_num]
    voxels_momentum = voxels_momentum[:voxel_num]
    voxels_GV_grad = voxels_GV_grad[:voxel_num]
    voxels_grad_points = voxels_grad_points[:voxel_num]
    voxels_points_adv = voxels_points_adv[:voxel_num]
    voxels_v = voxels_v[:voxel_num]
    return voxels, coors, num_points_per_voxel, voxels_innocent_ori, \
           voxels_momentum, voxels_GV_grad, voxels_grad_points, voxels_points_adv, voxels_v


def points_to_voxel_for_iterAttack_momentum_V(
        points, voxel_size, coors_range, max_points=35, reverse_index=True, max_voxels=20000,
        points_innocent_ori=np.array([]), momentum=np.array([]), v=np.array([]),
):
    """convert kitti points(N, >=3) to voxels. This version calculate
    everything in one loop. now it takes only 4.2ms(complete point cloud)
    with jit and 3.2ghz cpu.(don't calculate other features)
    ...
    coors = np.zeros(shape=(max_voxels, 3), dtype=np.int32)
    if reverse_index:
        voxel_num = _points_to_voxel_reverse_kernel_for_iterAttack_momentum_V(
            points,
            voxel_size,
            coors_range,
            num_points_per_voxel,
