...
class Logger(object):
    def __init__(self, dir='./', filename="Default.log"):
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
        self.terminal = sys.stdout
        self.log = open(os.path.join(dir, filename), "a", encoding='utf8')
...
def eval_main(nusc, eval_version, res_path, eval_set, output_dir, **kwargs):
    ...
    is_evaluate_ASR = kwargs['cfg'].get('is_evaluate_ASR', False)
    if is_evaluate_ASR:
        metrics_summary = nusc_eval.main(plot_examples=0, render_curves=False)
        return metrics_summary
    else:
        return nusc_eval.main(plot_examples=10, )
