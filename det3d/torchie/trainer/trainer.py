...
class Trainer(object):
    ...
    def batch_processor_inline(self, model, data, train_mode, **kwargs):
        ...
        # self.call_hook("after_data_to_device") # adv 注释

        # adv -------------------------------------------
        example['voxels'].requires_grad = True
        losses, predictions = model(example, return_loss=True, **kwargs)
        model.zero_grad()

        # grad_voxel = torch.autograd.grad(losses['loss'][0], example['voxels'], retain_graph=False, create_graph=False)[0]
        # # print('grad_voxel: {}'.format(grad_voxel))
        #
        # token = example['metadata'][0]['token']
        # save_dir = os.path.join('../work_dirs/nusc_centerpoint_voxelnet_0075voxel_fix_bn_z-adv', 'adv')
        # pc_10_samples = example['points'][0].detach().cpu().numpy()
        # save_point_cloud(pc_10_samples[:, :4], os.path.join(save_dir, str(token) + '-10.ply'))

        # save_point_cloud(pc_10_samples[:int(np.floor(pc_10_samples.shape[0]*0.1)), :4], os.path.join(save_dir, str(token) + '-1.ply'))
        ...
        if train_mode:
            ...
            # self.call_hook("after_forward") # adv 注释
            loss, log_vars = parse_second_losses(losses)
            del losses
            ...
            # self.call_hook("after_parse_loss") # adv 注释
    ...
    def train(self, data_loader, epoch, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            if self._inner_iter % 100 == 0:
                print('self._inner_iter: {}'.format(self._inner_iter))
            ...
            # self.call_hook("before_train_iter") # adv 注释
            ...
            # self.call_hook("after_train_iter") # adv 注释
            self._iter += 1
        ...
        result_dict, _ = dataset.evaluation(copy.deepcopy(predictions), output_dir=kwargs['args'].work_dir,
                                            testset=False, **kwargs)
        # adv ------------------------------------
            ...
            for i, flow in enumerate(workflow):
                ...
                for _ in range(epochs):
                    if mode == "train" and self.epoch >= max_epochs:
                        ...
                        epoch_runner(data_loaders[i], self.epoch, **kwargs)  # Trainer.train
